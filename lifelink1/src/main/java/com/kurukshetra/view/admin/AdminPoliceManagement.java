...
// public class AdminPoliceManagement {
...
public class AdminPoliceManagement {
    ...
    public VBox getPoliceManagement() {
        ...
        Button addPoliceButton = new Button("+   Add Police Unit");
        addPoliceButton.setPrefWidth(160);
        addPoliceButton.setPrefHeight(42);
        addPoliceButton.setStyle(PRIMARY_BUTTON_STYLE);

        Region headerSpacer = new Region();
        HBox.setHgrow(headerSpacer, Priority.ALWAYS);

        HBox header = new HBox(headingBox, headerSpacer, addPoliceButton);
        header.setAlignment(Pos.CENTER_LEFT);
        ...
        totalPoliceCard.setPadding(new Insets(18));
        totalPoliceCard.setPrefHeight(120);
        totalPoliceCard.setStyle(BASE_CARD_STYLE);

        Text totalPoliceTitle = new Text("Total Police Units");
        totalPoliceTitle.setStyle(FONT_STACK + "-fx-font-size: 13px; -fx-fill: " + TEXT_SECONDARY + "; -fx-font-weight: bold;");

        Text totalPoliceValue = new Text("42");
        totalPoliceValue.setStyle(FONT_STACK + "-fx-font-size: 28px; -fx-font-weight: bold; -fx-fill: " + TEXT_PRIMARY + ";");

        Text totalPoliceInfo = new Text("Registered units");
        totalPoliceInfo.setStyle(FONT_STACK + "-fx-font-size: 11px; -fx-fill: " + TEXT_MUTED + ";");

        totalPoliceCard.getChildren().addAll(totalPoliceTitle, totalPoliceValue, totalPoliceInfo);
        HBox.setHgrow(totalPoliceCard, Priority.ALWAYS);

        VBox activePoliceCard = new VBox(7);
        activePoliceCard.setPadding(new Insets(18));
        activePoliceCard.setPrefHeight(120);
        activePoliceCard.setStyle(BASE_CARD_STYLE);

        Text activePoliceTitle = new Text("Active Units");
        activePoliceTitle.setStyle(FONT_STACK + "-fx-font-size: 13px; -fx-fill: " + TEXT_SECONDARY + "; -fx-font-weight: bold;");

        Text activePoliceValue = new Text("28");
        activePoliceValue.setStyle(FONT_STACK + "-fx-font-size: 28px; -fx-font-weight: bold; -fx-fill: " + PURPLE_DARK + ";");

        Text activePoliceInfo = new Text("Currently deployed");
        activePoliceInfo.setStyle(FONT_STACK + "-fx-font-size: 11px; -fx-fill: " + SUCCESS_TEXT + "; -fx-font-weight: bold;");

        activePoliceCard.getChildren().addAll(activePoliceTitle, activePoliceValue, activePoliceInfo);
        HBox.setHgrow(activePoliceCard, Priority.ALWAYS);

        VBox availablePoliceCard = new VBox(7);
        availablePoliceCard.setPadding(new Insets(18));
        availablePoliceCard.setPrefHeight(120);
        availablePoliceCard.setStyle(BASE_CARD_STYLE);

        Text availablePoliceTitle = new Text("Available Units");
        availablePoliceTitle.setStyle(FONT_STACK + "-fx-font-size: 13px; -fx-fill: " + TEXT_SECONDARY + "; -fx-font-weight: bold;");

        Text availablePoliceValue = new Text("14");
        availablePoliceValue.setStyle(FONT_STACK + "-fx-font-size: 28px; -fx-font-weight: bold; -fx-fill: " + PURPLE_PRIMARY + ";");

        Text availablePoliceInfo = new Text("Ready for deployment");
        availablePoliceInfo.setStyle(FONT_STACK + "-fx-font-size: 11px; -fx-fill: " + PURPLE_DARK + ";");

        availablePoliceCard.getChildren().addAll(availablePoliceTitle, availablePoliceValue, availablePoliceInfo);
        HBox.setHgrow(availablePoliceCard, Priority.ALWAYS);

        VBox trafficCard = new VBox(7);
        trafficCard.setPadding(new Insets(18));
        trafficCard.setPrefHeight(120);
        trafficCard.setStyle(BASE_CARD_STYLE);

        Text trafficTitle = new Text("Traffic Support");
        trafficTitle.setStyle(FONT_STACK + "-fx-font-size: 13px; -fx-fill: " + TEXT_SECONDARY + "; -fx-font-weight: bold;");

        Text trafficValue = new Text("9");
        trafficValue.setStyle(FONT_STACK + "-fx-font-size: 28px; -fx-font-weight: bold; -fx-fill: " + TEXT_PRIMARY + ";");

        Text trafficInfo = new Text("Units managing routes");
        trafficInfo.setStyle(FONT_STACK + "-fx-font-size: 11px; -fx-fill: " + TEXT_MUTED + ";");

        trafficCard.getChildren().addAll(trafficTitle, trafficValue, trafficInfo);
        HBox.setHgrow(trafficCard, Priority.ALWAYS);
        ...
        // POLICE UNIT LIST
        VBox policeListCard = new VBox(15);
        policeListCard.setPadding(new Insets(22));
        policeListCard.setStyle(BASE_CARD_STYLE);

        Text listTitle = new Text("Police Units");
        listTitle.setStyle(FONT_STACK + "-fx-font-size: 18px; -fx-font-weight: bold; -fx-fill: " + TEXT_PRIMARY + ";");

        HBox police1 = createPoliceRow("Police Unit P-101", "Shivajinagar Zone", "ACTIVE", SUCCESS_TEXT);
        HBox police2 = createPoliceRow("Police Unit P-102", "Kothrud Zone", "AVAILABLE", PURPLE_DARK);
        HBox police3 = createPoliceRow("Police Unit P-103", "Hadapsar Zone", "ON DUTY", SUCCESS_TEXT);
        ...
        Button dispatchButton = new Button("Dispatch Police Unit");
        dispatchButton.setPrefWidth(180);
        dispatchButton.setPrefHeight(48);
        dispatchButton.setStyle(SECONDARY_BUTTON_STYLE);

        Button trafficButton = new Button("Manage Traffic");
        trafficButton.setPrefWidth(170);
        trafficButton.setPrefHeight(48);
        trafficButton.setStyle(SECONDARY_BUTTON_STYLE);

        Button emergencyRouteButton = new Button("Clear Emergency Route");
        emergencyRouteButton.setPrefWidth(190);
        emergencyRouteButton.setPrefHeight(48);
        emergencyRouteButton.setStyle(SECONDARY_BUTTON_STYLE);

        actionButtons.getChildren().addAll(dispatchButton, trafficButton, emergencyRouteButton);
        ...
        VBox result = new VBox(scrollPane);
        VBox.setVgrow(scrollPane, Priority.ALWAYS);
        result.setStyle("-fx-background-color: " + BG_PAGE + ";");

        return result;
    }

    private HBox createPoliceRow(String name, String zone, String status, String statusColor) {
        HBox row = new HBox(15);
        row.setPadding(new Insets(14, 16, 14, 16));
        row.setAlignment(Pos.CENTER_LEFT);
        row.setStyle(FONT_STACK + "-fx-background-color: #FAF8FF; -fx-background-radius: 12px; -fx-border-color: " + BORDER_COLOR + "; -fx-border-radius: 12px;");

        VBox info = new VBox(4);
        Text nameText = new Text(name);
        nameText.setStyle(FONT_STACK + "-fx-font-size: 13px; -fx-font-weight: bold; -fx-fill: " + TEXT_PRIMARY + ";");

        Text locText = new Text(zone);
        locText.setStyle(FONT_STACK + "-fx-font-size: 11px; -fx-fill: " + TEXT_MUTED + ";");

        info.getChildren().addAll(nameText, locText);

        Region spacer = new Region();
        HBox.setHgrow(spacer, Priority.ALWAYS);

        Text statusText = new Text(status);
        statusText.setStyle(FONT_STACK + "-fx-font-size: 11px; -fx-font-weight: bold; -fx-fill: " + statusColor + ";");

        Button viewButton = new Button("View");
        viewButton.setPrefWidth(70);
        viewButton.setStyle(FONT_STACK + "-fx-background-color: " + PURPLE_LIGHT + "; -fx-text-fill: " + PURPLE_DARK + "; -fx-font-size: 11px; -fx-font-weight: bold; -fx-background-radius: 8px; -fx-cursor: hand;");

        row.getChildren().addAll(info, spacer, statusText, viewButton);
        return row;
    }
}