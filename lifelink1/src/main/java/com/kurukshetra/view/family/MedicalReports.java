...
public class MedicalReports {
    ...
    private static final String TEXT = "#2B211D";
    private static final String SECONDARY = "#675B55";
    private static final String BORDER = "#EDE0D8";
    ...
    private VBox createMedicalReportsBody() {
        ...
        reportsContainer.setMinHeight(REPORT_HEIGHT);
        reportsContainer.setPrefHeight(REPORT_HEIGHT);
        reportsContainer.setMaxWidth(Double.MAX_VALUE);
    ...
    private VBox createMemberPanel() {
        ...
        panel.setFillWidth(true);
        panel.setStyle("-fx-background-color: #FEF7F2; -fx-border-color: " + BORDER + "; -fx-border-width: 0 1 0 0;");
        ...
        Button newButton = new Button("+ New");
        newButton.setPrefWidth(65);
        newButton.setPrefHeight(32);
        newButton.setStyle("-fx-background-color: " + PRIMARY_SELECTED + "; -fx-text-fill: " + PRIMARY_DARK + "; -fx-font-size: 11px; -fx-font-weight: bold; -fx-background-radius: 6px; -fx-cursor: hand;");
        newButton.setOnAction(e -> showMessage("New member button clicked."));

        heading.getChildren().addAll(title, space, newButton);
        panel.getChildren().add(heading);

        panel.getChildren().addAll(
                createMember("Member 1", true),
                createMember("Member 2", false),
                createMember("Member 3", false)
        );

        return panel;
    }

    private HBox createMember(String memberName, boolean selected) {
        HBox card = new HBox(8);
        card.setAlignment(Pos.CENTER_LEFT);
        card.setPrefWidth(MEMBER_CARD_WIDTH);
        card.setMinWidth(MEMBER_CARD_WIDTH);
        card.setMaxWidth(MEMBER_CARD_WIDTH);
        card.setPrefHeight(62);
        card.setMinHeight(62);
        card.setMaxHeight(62);
        card.setPadding(new Insets(9, 10, 9, 10));

        StackPane avatar = new StackPane();
        avatar.setPrefSize(38, 38);
        avatar.setMinSize(38, 38);
        avatar.setMaxSize(38, 38);
        avatar.setStyle("-fx-background-color: " + (selected ? "#F9E4D6" : "#F7EDE8") + "; -fx-background-radius: 50%;");

        Label dot = new Label("•");
        dot.setStyle("-fx-font-size: 13px; -fx-text-fill: " + (selected ? PRIMARY_DARK : SECONDARY) + ";");
        ...
        name.setMaxWidth(90);
        name.setWrapText(false);
        name.setStyle("-fx-font-size: 13px; -fx-font-weight: " + (selected ? "bold" : "normal") + "; -fx-text-fill: " + TEXT + ";");

        Region nameSpace = new Region();
        HBox.setHgrow(nameSpace, Priority.ALWAYS);

        card.getChildren().addAll(avatar, name, nameSpace);

        if (selected) {
            card.setStyle(
                    "-fx-background-color: " + PRIMARY_SELECTED + ";" +
        ...
        card.setOnMouseClicked(e -> animateNode(card));
        return card;
    }
    ...
    private VBox createDetailsPanel() {
        ...
        details.setPadding(new Insets(22, 24, 22, 24));
        details.setStyle("-fx-background-color: " + SURFACE + ";");

        Label title = new Label("Details");
        title.setStyle("-fx-font-size: 19px; -fx-font-weight: bold; -fx-text-fill: " + TEXT + ";");
        details.getChildren().addAll(title, createDivider());

        HBox nameRow = new HBox(14);
        VBox nameBox = createFormBox("Name", "Yashraj Pisal");
        VBox bloodBox = createFormBox("BG", "O+");
        HBox.setHgrow(nameBox, Priority.ALWAYS);
        bloodBox.setPrefWidth(90);
        bloodBox.setMinWidth(90);
        bloodBox.setMaxWidth(90);
        nameRow.getChildren().addAll(nameBox, bloodBox);

        HBox allergyRow = new HBox(14);
        VBox allergyBox = createFormBox("Allergies", "Penicillin");
        VBox weightBox = createFormBox("Wt", "65 kg");
        HBox.setHgrow(allergyBox, Priority.ALWAYS);
        HBox.setHgrow(weightBox, Priority.ALWAYS);
        allergyRow.getChildren().addAll(allergyBox, weightBox);
        ...
        reportButton.setPrefWidth(145);
        reportButton.setPrefHeight(42);
        reportButton.setStyle("-fx-background-color: " + PRIMARY_SELECTED + "; -fx-text-fill: " + PRIMARY_DARK + "; -fx-font-size: 12px; -fx-font-weight: bold; -fx-background-radius: 7px; -fx-cursor: hand;");
        reportButton.setOnAction(e -> showMessage("Medical report opened."));
        phoneRow.getChildren().addAll(phoneBox, reportButton);

        HBox saveRow = new HBox();
        saveRow.setAlignment(Pos.CENTER_RIGHT);
        Button saveButton = createPrimaryButton("Save");
        saveButton.setPrefWidth(84);
        saveButton.setOnAction(e -> showMessage("Medical details saved."));
        saveRow.getChildren().add(saveButton);

        details.getChildren().addAll(nameRow, allergyRow, phoneRow, saveRow);

        Region verticalSpace = new Region();
        VBox.setVgrow(verticalSpace, Priority.ALWAYS);
        details.getChildren().addAll(verticalSpace, createAllReportSection());
    ...
    private VBox createFormBox(String labelText, String value) {
        ...
        field.setMinHeight(44);
        field.setMaxWidth(Double.MAX_VALUE);
        field.setStyle("-fx-background-color: #FEF7F2; -fx-border-color: " + BORDER + "; -fx-border-width: 1px; -fx-border-radius: 7px; -fx-background-radius: 7px; -fx-font-size: 13px; -fx-text-fill: " + TEXT + "; -fx-padding: 0 12px;");
        box.getChildren().addAll(label, field);
        return box;
    }
    ...
    private VBox createAllReportSection() {
        ...
        reportType.setStyle("-fx-background-color: #FEF7F2; -fx-border-color: " + BORDER + "; -fx-border-radius: 7px; -fx-background-radius: 7px;");

        Label selectText = new Label("Select Report Type");
        selectText.setStyle("-fx-font-size: 12px; -fx-text-fill: " + TEXT + ";");
        reportType.getChildren().add(selectText);

        Button uploadButton = new Button("☁  Upload File");
        uploadButton.setPrefWidth(130);
        uploadButton.setPrefHeight(48);
        uploadButton.setStyle("-fx-background-color: white; -fx-border-color: " + PRIMARY + "; -fx-border-style: dashed; -fx-border-width: 1.5; -fx-border-radius: 7px; -fx-background-radius: 7px; -fx-text-fill: " + PRIMARY + "; -fx-font-size: 11px; -fx-font-weight: bold; -fx-cursor: hand;");
        uploadButton.setOnAction(e -> chooseReportFile());

        uploadRow.getChildren().addAll(reportType, uploadButton);
        section.getChildren().addAll(title, uploadRow);
        return section;
    }
    ...
    private Button createPrimaryButton(String text) {
        Button button = new Button(text);
        button.setPrefHeight(42);
        button.setStyle("-fx-background-color: " + PRIMARY + "; -fx-text-fill: white; -fx-font-size: 12px; -fx-font-weight: bold; -fx-background-radius: 7px; -fx-cursor: hand;");
        button.setOnMouseEntered(e -> button.setStyle("-fx-background-color: " + PRIMARY_DARK + "; -fx-text-fill: white; -fx-font-size: 12px; -fx-font-weight: bold; -fx-background-radius: 7px; -fx-cursor: hand;"));
        button.setOnMouseExited(e -> button.setStyle("-fx-background-color: " + PRIMARY + "; -fx-text-fill: white; -fx-font-size: 12px; -fx-font-weight: bold; -fx-background-radius: 7px; -fx-cursor: hand;"));
        return button;
    }
