...
public class HospitalPatientRecords {
    ...
    public VBox getPatientRecords() {
        ...
        Button notificationButton = new Button("🔔");
        notificationButton.setStyle("-fx-background-color: transparent; -fx-font-size: 18px; -fx-cursor: hand;");

        Button settingButton = new Button("⚙");
        settingButton.setStyle("-fx-background-color: transparent; -fx-font-size: 18px; -fx-cursor: hand;");

        Label profile = new Label("Dr. Sarah Jenkins");
        profile.setStyle("-fx-font-size: 14px; -fx-font-weight: bold; -fx-text-fill: " + TEXT_PRIMARY + ";");

        topBar.getChildren().addAll(searchField, notificationButton, settingButton, profile);

        // ==================== PAGE HEADER ====================
        VBox pageHeader = new VBox(4);
        pageHeader.setPadding(new Insets(24, 24, 16, 24));
        ...
        // ==================== ACTION BAR ====================
        HBox actionBar = new HBox();
        actionBar.setPadding(new Insets(0, 24, 16, 24));
        actionBar.setSpacing(10);
        actionBar.setAlignment(Pos.CENTER_RIGHT);

        Button filterButton = new Button("☷  Filter");
        filterButton.setPrefHeight(38);
        filterButton.setStyle("-fx-background-color: " + SURFACE + "; -fx-border-color: " + BORDER_COLOR + "; -fx-border-radius: 8px; -fx-background-radius: 8px; -fx-font-size: 12px; -fx-font-weight: bold; -fx-text-fill: " + TEXT_PRIMARY + "; -fx-cursor: hand;");

        Button newPatientButton = new Button("+  New Patient");
        newPatientButton.setPrefHeight(38);
        newPatientButton.setStyle("-fx-background-color: " + PRIMARY_TEAL + "; -fx-text-fill: white; -fx-background-radius: 8px; -fx-font-size: 12px; -fx-font-weight: bold; -fx-padding: 0 16px; -fx-cursor: hand;");
        newPatientButton.setOnMouseEntered(e -> newPatientButton.setStyle("-fx-background-color: " + TEAL_DARK + "; -fx-text-fill: white; -fx-background-radius: 8px; -fx-font-size: 12px; -fx-font-weight: bold; -fx-padding: 0 16px; -fx-cursor: hand;"));
        newPatientButton.setOnMouseExited(e -> newPatientButton.setStyle("-fx-background-color: " + PRIMARY_TEAL + "; -fx-text-fill: white; -fx-background-radius: 8px; -fx-font-size: 12px; -fx-font-weight: bold; -fx-padding: 0 16px; -fx-cursor: hand;"));

        actionBar.getChildren().addAll(filterButton, newPatientButton);

        // ==================== TABLE ====================
        VBox tableBox = new VBox();
        tableBox.setStyle(CARD_STYLE);

        HBox tableHeader = new HBox();
        tableHeader.setPadding(new Insets(12, 16, 12, 16));
        tableHeader.setAlignment(Pos.CENTER_LEFT);
        tableHeader.setStyle("-fx-background-color: " + TEAL_VERY_LIGHT + "; -fx-border-color: " + BORDER_COLOR + "; -fx-border-width: 0px 0px 1px 0px; -fx-background-radius: 12px 12px 0 0;");

        tableHeader.getChildren().addAll(
            createPatientHeader("PATIENT ID", 95),
            createPatientHeader("NAME", 150),
            createPatientHeader("BLOOD", 75),
            createPatientHeader("ALLERGIES", 150),
            createPatientHeader("CONDITION", 150),
            createPatientHeader("LAST VISIT", 110),
            createPatientHeader("ACTIONS", 180)
        );

        HBox row1 = createPatientRow("#LL-8492", "Sarah Jenkins\n42 yrs • Female", "A+", STATUS_WARN_BG, STATUS_WARN_TEXT, "Penicillin, Latex", "Type 2 Diabetes", "24 Oct 2023", true);
        HBox row2 = createPatientRow("#LL-1023", "Robert Chen\n58 yrs • Male", "O-", STATUS_DANGER_BG, STATUS_DANGER_TEXT, "Shellfish", "Hypertension", "12 Nov 2023", false);
        HBox row3 = createPatientRow("#LL-4512", "Emily Rodriguez\n29 yrs • Female", "B+", TEAL_VERY_LIGHT, TEAL_DARK, "None", "Asthma", "05 Nov 2023", true);
        HBox row4 = createPatientRow("#LL-9932", "David Wilson\n45 yrs • Male", "AB+", TEAL_LIGHT, PRIMARY_TEAL, "Dust Mites", "Chronic Back Pain", "18 Nov 2023", false);
        HBox row5 = createPatientRow("#LL-5221", "Aisha Khan\n34 yrs • Female", "O+", STATUS_WARN_BG, STATUS_WARN_TEXT, "Pollen", "Gastroenteritis", "02 Nov 2023", true);

        tableBox.getChildren().addAll(tableHeader, row1, row2, row3, row4, row5);

        VBox tableWrapper = new VBox(tableBox);
        tableWrapper.setPadding(new Insets(0, 24, 0, 24));
        ...
        Label showing = new Label("Showing 1 to 10 of 1,248 patients");
        showing.setStyle("-fx-font-size: 12px; -fx-text-fill: " + TEXT_MUTED + ";");
        HBox.setHgrow(showing, Priority.ALWAYS);
        ...
        previous.setStyle("-fx-background-color: transparent; -fx-font-size: 16px; -fx-text-fill: " + TEXT_MUTED + "; -fx-cursor: hand;");
        page1.setStyle("-fx-background-color: " + PRIMARY_TEAL + "; -fx-text-fill: white; -fx-background-radius: 6px; -fx-font-weight: bold; -fx-cursor: hand;");
        page2.setStyle("-fx-background-color: transparent; -fx-font-size: 13px; -fx-text-fill: " + TEXT_MUTED + "; -fx-cursor: hand;");
        page3.setStyle("-fx-background-color: transparent; -fx-font-size: 13px; -fx-text-fill: " + TEXT_MUTED + "; -fx-cursor: hand;");
        next.setStyle("-fx-background-color: transparent; -fx-font-size: 16px; -fx-text-fill: " + TEXT_MUTED + "; -fx-cursor: hand;");

        pagination.getChildren().addAll(showing, previous, page1, page2, page3, next);

        VBox leftContent = new VBox(pageHeader, actionBar, tableWrapper, pagination);

        // ==================== MEDICAL SUMMARY ====================
        VBox medicalSummary = new VBox();
        medicalSummary.setPrefWidth(360);
        medicalSummary.setPadding(new Insets(24));
        medicalSummary.setSpacing(16);
        medicalSummary.setStyle("-fx-background-color: " + SURFACE + "; -fx-border-color: " + BORDER_COLOR + "; -fx-border-width: 0px 0px 0px 1px;");

        Label summaryTitle = new Label("Medical Summary");
        summaryTitle.setStyle("-fx-font-size: 17px; -fx-font-weight: bold; -fx-text-fill: " + TEXT_PRIMARY + ";");

        VBox patientInfo = new VBox();
        patientInfo.setAlignment(Pos.CENTER);
        patientInfo.setSpacing(6);

        Label patientIcon = new Label("👤");
        patientIcon.setPrefSize(80, 80);
        patientIcon.setAlignment(Pos.CENTER);
        patientIcon.setStyle("-fx-background-color: " + TEAL_VERY_LIGHT + "; -fx-background-radius: 16px; -fx-border-color: " + BORDER_COLOR + "; -fx-border-radius: 16px; -fx-font-size: 38px;");

        Label summaryName = new Label("Robert Chen");
        summaryName.setStyle("-fx-font-size: 20px; -fx-font-weight: bold; -fx-text-fill: " + TEXT_PRIMARY + ";");

        Label summaryId = new Label("#LL-1023");
        summaryId.setStyle("-fx-font-size: 12px; -fx-text-fill: " + PRIMARY_TEAL + "; -fx-font-weight: bold;");

        patientInfo.getChildren().addAll(patientIcon, summaryName, summaryId);

        // Vitals
        VBox heartRate = createVitalBox("♥  HEART RATE", "72 bpm");
        VBox temperature = createVitalBox("🌡  TEMP", "98.6 °F");
        HBox vitalsRow1 = new HBox(10, heartRate, temperature);

        VBox bp = createVitalBox("↕  BP", "120/80 mmHg");
        VBox spo2 = createVitalBox("♨  SPO2", "99 %");
        HBox vitalsRow2 = new HBox(10, bp, spo2);

        // Physician
        VBox physicianBox = new VBox(6);
        Label physicianTitle = new Label("PRIMARY PHYSICIAN");
        physicianTitle.setStyle("-fx-font-size: 10px; -fx-font-weight: bold; -fx-text-fill: " + TEXT_MUTED + ";");

        HBox physician = new HBox();
        physician.setPadding(new Insets(12));
        physician.setSpacing(12);
        physician.setAlignment(Pos.CENTER_LEFT);
        physician.setStyle("-fx-background-color: " + TEAL_PALE + "; -fx-border-color: " + BORDER_COLOR + "; -fx-border-radius: 10px; -fx-background-radius: 10px;");

        Label doctorIcon = new Label("⚕");
        doctorIcon.setStyle("-fx-font-size: 20px; -fx-text-fill: " + PRIMARY_TEAL + ";");

        Label doctorName = new Label("Dr. Elena Vance");
        doctorName.setStyle("-fx-font-size: 13px; -fx-font-weight: bold; -fx-text-fill: " + TEXT_PRIMARY + ";");

        Label doctorRole = new Label("Chief Cardiologist");
        doctorRole.setStyle("-fx-font-size: 11px; -fx-text-fill: " + TEXT_SECONDARY + ";");

        VBox doctorDetails = new VBox(2, doctorName, doctorRole);
        physician.getChildren().addAll(doctorIcon, doctorDetails);
        physicianBox.getChildren().addAll(physicianTitle, physician);

        // Recent Records
        VBox recentRecords = new VBox(6);
        Label recentTitle = new Label("RECENT RECORDS");
        recentTitle.setStyle("-fx-font-size: 10px; -fx-font-weight: bold; -fx-text-fill: " + TEXT_MUTED + ";");

        HBox record1 = createRecordRow("▣  Cardiac MRI Scan", "Nov 10");
        HBox record2 = createRecordRow("▣  Lipid Profile Test", "Nov 08");
        HBox record3 = createRecordRow("▣  Flu Vaccination", "Oct 15");

        recentRecords.getChildren().addAll(recentTitle, record1, record2, record3);

        Button analyticsButton = new Button("⌁  Full Medical Analytics");
        analyticsButton.setMaxWidth(Double.MAX_VALUE);
        analyticsButton.setPrefHeight(42);
        analyticsButton.setStyle("-fx-background-color: " + SURFACE + "; -fx-border-color: " + PRIMARY_TEAL + "; -fx-border-width: 1.5px; -fx-border-radius: 10px; -fx-background-radius: 10px; -fx-text-fill: " + PRIMARY_TEAL + "; -fx-font-weight: bold; -fx-font-size: 12px; -fx-cursor: hand;");
        analyticsButton.setOnMouseEntered(e -> analyticsButton.setStyle("-fx-background-color: " + TEAL_VERY_LIGHT + "; -fx-border-color: " + PRIMARY_TEAL + "; -fx-border-width: 1.5px; -fx-border-radius: 10px; -fx-background-radius: 10px; -fx-text-fill: " + PRIMARY_TEAL + "; -fx-font-weight: bold; -fx-font-size: 12px; -fx-cursor: hand;"));
        analyticsButton.setOnMouseExited(e -> analyticsButton.setStyle("-fx-background-color: " + SURFACE + "; -fx-border-color: " + PRIMARY_TEAL + "; -fx-border-width: 1.5px; -fx-border-radius: 10px; -fx-background-radius: 10px; -fx-text-fill: " + PRIMARY_TEAL + "; -fx-font-weight: bold; -fx-font-size: 12px; -fx-cursor: hand;"));

        medicalSummary.getChildren().addAll(summaryTitle, patientInfo, vitalsRow1, vitalsRow2, physicianBox, recentRecords, analyticsButton);

        HBox content = new HBox(leftContent, medicalSummary);
        HBox.setHgrow(leftContent, Priority.ALWAYS);
    ...
    private Label createPatientHeader(String text, double width) {
    ...
    private HBox createPatientRow(String id, String name, String blood, String bloodBg, String bloodColor, String allergy, String condition, String visit, boolean alternate) {
        Label idLbl = new Label(id);
        idLbl.setPrefWidth(95);
        idLbl.setStyle("-fx-text-fill: " + PRIMARY_TEAL + "; -fx-font-size: 12px; -fx-font-weight: bold;");
        ...
        bloodLbl.setStyle("-fx-background-color: " + bloodBg + "; -fx-text-fill: " + bloodColor + "; -fx-padding: 3px 7px; -fx-background-radius: 5px; -fx-font-weight: bold; -fx-font-size: 11px;");
        HBox bloodBox = new HBox(bloodLbl);
        bloodBox.setPrefWidth(75);
        bloodBox.setAlignment(Pos.CENTER_LEFT);

        Label allergyLbl = new Label(allergy);
        allergyLbl.setPrefWidth(150);
        allergyLbl.setStyle("-fx-text-fill: " + TEXT_SECONDARY + "; -fx-font-size: 12px;");

        Label conditionLbl = new Label(condition);
        conditionLbl.setPrefWidth(150);
        conditionLbl.setStyle("-fx-text-fill: " + TEXT_SECONDARY + "; -fx-font-size: 12px;");

        Label visitLbl = new Label(visit);
        visitLbl.setPrefWidth(110);
        visitLbl.setStyle("-fx-text-fill: " + TEXT_MUTED + "; -fx-font-size: 12px;");

        String btnStyle = "-fx-background-color: transparent; -fx-text-fill: " + PRIMARY_TEAL + "; -fx-font-size: 15px; -fx-cursor: hand;";
        Button h = new Button("↶"); h.setStyle(btnStyle);
        Button r = new Button("◉"); r.setStyle(btnStyle);
        Button d = new Button("↓"); d.setStyle(btnStyle);
        Button u = new Button("↑"); u.setStyle(btnStyle);

        HBox actions = new HBox(2, h, r, d, u);
        actions.setPrefWidth(180);
        actions.setAlignment(Pos.CENTER_RIGHT);

        HBox row = new HBox(idLbl, nameLbl, bloodBox, allergyLbl, conditionLbl, visitLbl, actions);
        row.setPadding(new Insets(12, 16, 12, 16));
        row.setAlignment(Pos.CENTER_LEFT);
        row.setStyle("-fx-background-color: " + (alternate ? SURFACE : "#F8FCFC") + "; -fx-border-color: " + BORDER_COLOR + "; -fx-border-width: 0px 0px 1px 0px;");
        return row;
    }

    private VBox createVitalBox(String title, String val) {
        VBox box = new VBox(2);
        box.setPadding(new Insets(10, 12, 10, 12));
        box.setPrefWidth(145);
        box.setStyle("-fx-background-color: " + TEAL_PALE + "; -fx-border-color: " + BORDER_COLOR + "; -fx-border-radius: 10px; -fx-background-radius: 10px;");

        Label t = new Label(title);
        t.setStyle("-fx-font-size: 9px; -fx-font-weight: bold; -fx-text-fill: " + TEXT_MUTED + ";");

        Label v = new Label(val);
        v.setStyle("-fx-font-size: 15px; -fx-font-weight: bold; -fx-text-fill: " + TEXT_PRIMARY + ";");

        box.getChildren().addAll(t, v);
        return box;
    }

    private HBox createRecordRow(String name, String date) {
        HBox row = new HBox();
        row.setPadding(new Insets(8, 10, 8, 10));
        row.setAlignment(Pos.CENTER_LEFT);
        row.setStyle("-fx-background-color: " + TEAL_PALE + "; -fx-background-radius: 6px; -fx-border-color: " + BORDER_COLOR + "; -fx-border-radius: 6px;");

        Label n = new Label(name);
        n.setStyle("-fx-font-size: 12px; -fx-text-fill: " + TEXT_PRIMARY + ";");
        HBox.setHgrow(n, Priority.ALWAYS);

        Label d = new Label(date);
        d.setStyle("-fx-font-size: 11px; -fx-text-fill: " + TEXT_MUTED + ";");

        row.getChildren().addAll(n, d);
        return row;
    }
}