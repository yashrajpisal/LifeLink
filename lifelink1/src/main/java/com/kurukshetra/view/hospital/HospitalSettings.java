...
public class HospitalSettings {
    ...
    public VBox getSettings() {
        ...
        Button settingsButton = new Button("⚙");
        settingsButton.setPrefSize(38, 38);
        settingsButton.setStyle("-fx-background-color: " + TEAL_PALE + "; -fx-font-size: 18px; -fx-background-radius: 50%; -fx-cursor: hand;");

        Label adminView = new Label("Admin View");
        adminView.setStyle("-fx-font-size: 15px; -fx-font-weight: bold; -fx-text-fill: " + TEXT_PRIMARY + ";");
        ...
        // ========================= PAGE CONTENT =========================
        VBox pageContent = new VBox();
        pageContent.setPadding(new Insets(26));
        ...
        // ========================= HOSPITAL INFORMATION =========================
        VBox hospitalSection = new VBox(14);
        hospitalSection.setPadding(new Insets(20));
        hospitalSection.setStyle(CARD_STYLE);

        Label hospitalTitle = new Label("Hospital Information");
        hospitalTitle.setStyle("-fx-font-size: 17px; -fx-font-weight: bold; -fx-text-fill: " + TEXT_PRIMARY + ";");

        Label hospitalDescription = new Label("Update basic information about your hospital.");
        hospitalDescription.setStyle("-fx-font-size: 12px; -fx-text-fill: " + TEXT_SECONDARY + ";");

        VBox hospitalNameBox = new VBox(5);
        Label hospitalNameLabel = new Label("Hospital Name");
        hospitalNameLabel.setStyle("-fx-font-size: 12px; -fx-font-weight: bold; -fx-text-fill: " + TEXT_SECONDARY + ";");

        TextField hospitalName = new TextField("LifeLink Central Hospital");
        hospitalName.setPrefHeight(38);
        hospitalName.setStyle(FIELD_STYLE);
        hospitalNameBox.getChildren().addAll(hospitalNameLabel, hospitalName);

        VBox hospitalEmailBox = new VBox(5);
        Label hospitalEmailLabel = new Label("Hospital Email");
        hospitalEmailLabel.setStyle("-fx-font-size: 12px; -fx-font-weight: bold; -fx-text-fill: " + TEXT_SECONDARY + ";");

        TextField hospitalEmail = new TextField("admin@lifelinkhospital.com");
        hospitalEmail.setPrefHeight(38);
        hospitalEmail.setStyle(FIELD_STYLE);
        hospitalEmailBox.getChildren().addAll(hospitalEmailLabel, hospitalEmail);

        HBox hospitalRow = new HBox(16, hospitalNameBox, hospitalEmailBox);
        HBox.setHgrow(hospitalNameBox, Priority.ALWAYS);
        HBox.setHgrow(hospitalEmailBox, Priority.ALWAYS);

        VBox hospitalAddressBox = new VBox(5);
        Label hospitalAddressLabel = new Label("Hospital Address");
        hospitalAddressLabel.setStyle("-fx-font-size: 12px; -fx-font-weight: bold; -fx-text-fill: " + TEXT_SECONDARY + ";");

        TextField hospitalAddress = new TextField("Pune, Maharashtra, India");
        hospitalAddress.setPrefHeight(38);
        hospitalAddress.setStyle(FIELD_STYLE);
        hospitalAddressBox.getChildren().addAll(hospitalAddressLabel, hospitalAddress);

        Button saveHospitalButton = new Button("Save Changes");
        saveHospitalButton.setPrefHeight(38);
        saveHospitalButton.setStyle("-fx-background-color: " + PRIMARY_TEAL + "; -fx-text-fill: white; -fx-font-size: 12px; -fx-font-weight: bold; -fx-background-radius: 8px; -fx-padding: 8px 18px; -fx-cursor: hand;");
        saveHospitalButton.setOnMouseEntered(e -> saveHospitalButton.setStyle("-fx-background-color: " + TEAL_DARK + "; -fx-text-fill: white; -fx-font-size: 12px; -fx-font-weight: bold; -fx-background-radius: 8px; -fx-padding: 8px 18px; -fx-cursor: hand;"));
        saveHospitalButton.setOnMouseExited(e -> saveHospitalButton.setStyle("-fx-background-color: " + PRIMARY_TEAL + "; -fx-text-fill: white; -fx-font-size: 12px; -fx-font-weight: bold; -fx-background-radius: 8px; -fx-padding: 8px 18px; -fx-cursor: hand;"));

        hospitalSection.getChildren().addAll(hospitalTitle, hospitalDescription, hospitalRow, hospitalAddressBox, saveHospitalButton);

        // ========================= NOTIFICATION SETTINGS =========================
        VBox notificationSection = new VBox(12);
        notificationSection.setPadding(new Insets(20));
        notificationSection.setStyle(CARD_STYLE);

        Label notificationTitle = new Label("Notification Preferences");
        notificationTitle.setStyle("-fx-font-size: 17px; -fx-font-weight: bold; -fx-text-fill: " + TEXT_PRIMARY + ";");

        Label notificationDescription = new Label("Choose which hospital events should generate notifications.");
        notificationDescription.setStyle("-fx-font-size: 12px; -fx-text-fill: " + TEXT_SECONDARY + ";");

        String checkStyle = "-fx-font-size: 13px; -fx-text-fill: " + TEXT_PRIMARY + ";";
        CheckBox emergencyNotifications = new CheckBox("Emergency Requests"); emergencyNotifications.setSelected(true); emergencyNotifications.setStyle(checkStyle);
        CheckBox ambulanceNotifications = new CheckBox("Ambulance Arrival Notifications"); ambulanceNotifications.setSelected(true); ambulanceNotifications.setStyle(checkStyle);
        CheckBox resourceNotifications = new CheckBox("Low Resource Alerts"); resourceNotifications.setSelected(true); resourceNotifications.setStyle(checkStyle);
        CheckBox doctorNotifications = new CheckBox("Doctor Assignment Notifications"); doctorNotifications.setSelected(true); doctorNotifications.setStyle(checkStyle);
        CheckBox otNotifications = new CheckBox("Operation Theatre Updates"); otNotifications.setSelected(true); otNotifications.setStyle(checkStyle);

        notificationSection.getChildren().addAll(notificationTitle, notificationDescription, emergencyNotifications, ambulanceNotifications, resourceNotifications, doctorNotifications, otNotifications);

        // ========================= EMERGENCY SETTINGS =========================
        VBox emergencySection = new VBox(12);
        emergencySection.setPadding(new Insets(20));
        emergencySection.setStyle(CARD_STYLE);

        Label emergencyTitle = new Label("Emergency Settings");
        emergencyTitle.setStyle("-fx-font-size: 17px; -fx-font-weight: bold; -fx-text-fill: " + TEXT_PRIMARY + ";");

        Label emergencyDescription = new Label("Configure how critical emergency requests are handled.");
        emergencyDescription.setStyle("-fx-font-size: 12px; -fx-text-fill: " + TEXT_SECONDARY + ";");

        CheckBox automaticEmergency = new CheckBox("Enable automatic emergency alerts"); automaticEmergency.setSelected(true); automaticEmergency.setStyle(checkStyle);
        CheckBox ambulanceAutoAssign = new CheckBox("Automatically notify ambulance team"); ambulanceAutoAssign.setSelected(true); ambulanceAutoAssign.setStyle(checkStyle);
        CheckBox doctorAlert = new CheckBox("Alert available emergency doctors"); doctorAlert.setSelected(true); doctorAlert.setStyle(checkStyle);
        CheckBox icuAlert = new CheckBox("Notify when ICU capacity reaches critical level"); icuAlert.setSelected(true); icuAlert.setStyle(checkStyle);

        emergencySection.getChildren().addAll(emergencyTitle, emergencyDescription, automaticEmergency, ambulanceAutoAssign, doctorAlert, icuAlert);

        // ========================= SYSTEM SETTINGS =========================
        VBox systemSection = new VBox(14);
        systemSection.setPadding(new Insets(20));
        systemSection.setStyle(CARD_STYLE);

        Label systemTitle = new Label("System Preferences");
        systemTitle.setStyle("-fx-font-size: 17px; -fx-font-weight: bold; -fx-text-fill: " + TEXT_PRIMARY + ";");

        Label systemDescription = new Label("Configure general application preferences.");
        systemDescription.setStyle("-fx-font-size: 12px; -fx-text-fill: " + TEXT_SECONDARY + ";");

        VBox languageBox = new VBox(5);
        Label languageLabel = new Label("Language");
        languageLabel.setStyle("-fx-font-size: 12px; -fx-font-weight: bold; -fx-text-fill: " + TEXT_SECONDARY + ";");

        ComboBox<String> languageCombo = new ComboBox<>();
        languageCombo.getItems().addAll("English", "Hindi", "Marathi");
        languageCombo.setValue("English");
        languageCombo.setPrefWidth(220);
        languageCombo.setPrefHeight(38);
        languageCombo.setStyle("-fx-background-color: " + SURFACE + "; -fx-border-color: " + BORDER_COLOR + "; -fx-border-radius: 8px; -fx-background-radius: 8px; -fx-font-size: 12px;");
        languageBox.getChildren().addAll(languageLabel, languageCombo);

        VBox refreshBox = new VBox(5);
        Label refreshLabel = new Label("Dashboard Refresh Interval");
        refreshLabel.setStyle("-fx-font-size: 12px; -fx-font-weight: bold; -fx-text-fill: " + TEXT_SECONDARY + ";");

        ComboBox<String> refreshCombo = new ComboBox<>();
        refreshCombo.getItems().addAll("10 Seconds", "30 Seconds", "1 Minute", "5 Minutes");
        refreshCombo.setValue("30 Seconds");
        refreshCombo.setPrefWidth(220);
        refreshCombo.setPrefHeight(38);
        refreshCombo.setStyle("-fx-background-color: " + SURFACE + "; -fx-border-color: " + BORDER_COLOR + "; -fx-border-radius: 8px; -fx-background-radius: 8px; -fx-font-size: 12px;");
        refreshBox.getChildren().addAll(refreshLabel, refreshCombo);

        HBox systemRow = new HBox(25, languageBox, refreshBox);
        systemSection.getChildren().addAll(systemTitle, systemDescription, systemRow);

        // ========================= SECURITY SETTINGS =========================
        VBox securitySection = new VBox(12);
        securitySection.setPadding(new Insets(20));
        securitySection.setStyle(CARD_STYLE);

        Label securityTitle = new Label("Security");
        securityTitle.setStyle("-fx-font-size: 17px; -fx-font-weight: bold; -fx-text-fill: " + TEXT_PRIMARY + ";");

        Label securityDescription = new Label("Manage administrator account and system security preferences.");
        securityDescription.setStyle("-fx-font-size: 12px; -fx-text-fill: " + TEXT_SECONDARY + ";");

        CheckBox twoFactor = new CheckBox("Enable two-factor authentication"); twoFactor.setSelected(true); twoFactor.setStyle(checkStyle);
        CheckBox sessionTimeout = new CheckBox("Automatically log out inactive sessions"); sessionTimeout.setSelected(true); sessionTimeout.setStyle(checkStyle);

        Button changePassword = new Button("Change Password");
        changePassword.setPrefHeight(38);
        changePassword.setStyle("-fx-background-color: " + SURFACE + "; -fx-border-color: " + BORDER_COLOR + "; -fx-border-radius: 8px; -fx-background-radius: 8px; -fx-text-fill: " + TEXT_PRIMARY + "; -fx-font-size: 12px; -fx-font-weight: bold; -fx-padding: 8px 18px; -fx-cursor: hand;");

        securitySection.getChildren().addAll(securityTitle, securityDescription, twoFactor, sessionTimeout, changePassword);
        ...
        cancelButton.setStyle("-fx-background-color: " + SURFACE + "; -fx-border-color: " + BORDER_COLOR + "; -fx-border-radius: 8px; -fx-background-radius: 8px; -fx-text-fill: " + TEXT_SECONDARY + "; -fx-font-size: 12px; -fx-font-weight: bold; -fx-padding: 8px 20px; -fx-cursor: hand;");

        Button saveButton = new Button("Save All Changes");
        saveButton.setPrefHeight(38);
        saveButton.setStyle("-fx-background-color: " + PRIMARY_TEAL + "; -fx-text-fill: white; -fx-background-radius: 8px; -fx-font-size: 12px; -fx-font-weight: bold; -fx-padding: 8px 20px; -fx-cursor: hand;");
        saveButton.setOnMouseEntered(e -> saveButton.setStyle("-fx-background-color: " + TEAL_DARK + "; -fx-text-fill: white; -fx-background-radius: 8px; -fx-font-size: 12px; -fx-font-weight: bold; -fx-padding: 8px 20px; -fx-cursor: hand;"));
        saveButton.setOnMouseExited(e -> saveButton.setStyle("-fx-background-color: " + PRIMARY_TEAL + "; -fx-text-fill: white; -fx-background-radius: 8px; -fx-font-size: 12px; -fx-font-weight: bold; -fx-padding: 8px 20px; -fx-cursor: hand;"));

        actionBar.getChildren().addAll(cancelButton, saveButton);

        pageContent.getChildren().addAll(
            pageHeader, hospitalSection, notificationSection, emergencySection,
            systemSection, securitySection, new Separator(), actionBar
        );
        ...
        VBox settingsPage = new VBox(topBar, scrollPane);
        VBox.setVgrow(scrollPane, Priority.ALWAYS);
